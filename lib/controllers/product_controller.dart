...
class ProductController extends GetxController {
  ...
  List<Product> _extraProductList=[];
  ...
  List<Product> get extraProductList => _extraProductList;
  ...
  int get popularLength =>_popularProductList.length;
  int get recommendedLength =>_recommendedProductList.length;
  int get extraLength =>extraProductList.length;
  ...
  Future<void> getPopularProductList(bool reload) async {
    if(_popularProductList.isEmpty || reload) {
  ...
  Future<void> getExtraProductList(bool reload) async {
    if(_extraProductList.isEmpty || reload) {

      Response response = await productRepo.getExtraProductList();
      if (response.statusCode == 200) {
        _extraProductList = [];
        _extraProductList.addAll(ProductModel.fromJson(response.body).products);
